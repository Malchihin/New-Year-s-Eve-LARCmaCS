...
void FieldScene::drawChristmasTree()
{
    QGraphicsRectItem* trunk;
    QGraphicsRectItem* branch;
    QGraphicsRectItem* star;

    double centerX = 0;
    double centerY = 0;

    trunk = new QGraphicsRectItem(-8, -80, 16, 80);
    trunk->setBrush(QBrush(QColor(139, 69, 19, 255)));
    trunk->setPen(Qt::NoPen);
    ...
    for (int i = 0; i < 5; ++i) {
        branch = new QGraphicsRectItem(-branchW / 2, -branchH / 2, branchW, branchH);
        branch->setBrush(QBrush(branchColor));
        branch->setPen(Qt::NoPen);
        branch->setPos(centerX, centerY - branchY);
        this->addItem(branch);

    }

    QPolygonF starPolygon;
    starPolygon << QPointF(0, -40) << QPointF(-20, 20) << QPointF(20, 20);
    star = new QGraphicsRectItem(starPolygon);
    star->setBrush(QBrush(QColor(255, 215, 0, 255)));
    star->setPen(Qt::NoPen);
    star->setPos(centerX, centerY - 300); 
    this->addItem(star);
}